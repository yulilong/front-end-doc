...
const handleCreated = (editor: any) => {
  ...
  editor.getHtml() // 获取html文本内容
  editor.setHtml('<p>123</p>') // 设置文本内容
...
上传附件中，如果使用了自定义上传方法，在自定义上传成功后，需要主动吧更新后的变量传给父组件，
...
**注意**：这个手动同步文本内容不能使用异步`setTimeout` 方法，会导致手动选中附件导致附件消失。
