...
### 1.1 series[0].label.formatter 说明
...
let label = {
  ...
  formatter: function(params) {
    // 根据数值返回不同评级
    if (params.value < 60) return '不及格';
    if (params.value < 70) return '及格';
    if (params.value < 80) return '良好';
    return '优秀';
  }
}
// 带富文本的
...
- ‌`params` 参数‌：是一个对象，包含当前数据点的详细信息，常用属性包括：
  - `params.data`：当前数据项的值
  - `params.dataIndex`：当前数据项的索引值
  - `params.seriesName`：当前系列的名称
  - `params.name`：数据名（x轴坐标值）
  - `params.value`：数据值（y轴坐标值）
...
- 纯文本内容
- 包含 HTML 标签的富文本（需配合 `rich` 配置使用）4
- 使用模板变量如 `{c}` 表示数据值3

## 2. 一些实际例子

### 2.1 每个数据显示不同颜色，不显示Y轴信息
