...
const server = http.createServer((req, res) => {
    const { pathname } = url.parse(req.url, true);    // 获取文件名
    ...
    fs.stat(filepath, (err, stat) => {
        ...
        if (err) {
            ...
            // 缓存没有通过则返回资源，并加上 last-modified响应头，下次浏览器就会在请求头中带着 if-modified-since
                ...
                fs.readFile(filepath, 'binary', function (err, resource) {
                    if (err) {
                        res.writeHead(500);
                        // res.writeHead(500, { 'Content-Type': mime.getType(filepath) });
                        res.end();
                    } else {
                        res.writeHead(200);
...
[node实战前端缓存总结](https://www.cnblogs.com/Jiangchuanwei/p/10632477.html)

[深入node.js-浏览器缓存机制](https://segmentfault.com/a/1190000018055744)
