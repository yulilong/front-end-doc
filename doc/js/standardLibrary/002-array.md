...
2、`shift()`方法用于删除数组的第一个元素，并返回该元素。注意，该方法会改变原数组。
...
它的第一个参数为起始位置（从0开始），第二个参数为终止位置（但该位置的元素本身不包括在内）。如果省略第二个参数，则一直返回到原数组的最后一个成员。如果第一个参数也省略，则返回一个原数组的拷贝。
...
`map()` 方法会创建一个新数组，新数组的每个元素是原数组元素调用回调函数后的返回值。

- 参数
  - **回调函数 (callback)**：对每个数组元素执行的操作，必须返回一个值，作为新数组的元素。回调函数接收以下参数（按顺序）
    - `currentValue`：当前处理的元素。
    - `index`（可选）：当前元素的索引。
    - `array`（可选）：调用 `map` 的原数组本身。
  - **thisArg（可选）**：回调函数中 `this` 指向的对象（默认是 `undefined`）。
- 返回值
  - 返回一个**新数组**，每个元素是原数组元素经过回调函数处理后的结果。
  - 如果原数组是稀疏数组（有“空位”），`map` 会跳过空位，返回的数组也会保留空位。
- 其他注意事项
  - 如果对空数组调用 `map`，不会执行回调函数，直接返回一个新的空数组。
  - **map方法不会修改原数组**。
...
// 空数组 不执行
const emptyArr = [];
const result = emptyArr.map(x => x * 2); // result 是 []
```



### 3.11 forEach()：遍历数组，没有返回值
...
### 3.14 reduce()，reduceRight()：一次处理，累计为一个值
...
### 3.15 indexOf()，lastIndexOf()：查找元素在数组中第一次出现位置
